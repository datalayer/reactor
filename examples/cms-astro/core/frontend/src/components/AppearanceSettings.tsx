...
export function AppearanceSettings({websiteThemes,activeWebsiteTheme,onActivateWebsiteTheme}:{websiteThemes:WebsiteTheme[];activeWebsiteTheme?:string;onActivateWebsiteTheme:(theme:WebsiteTheme)=>void}):ReactElement{
 ...
 return <Box sx={{maxWidth:960,width:'100%',mx:'auto'}}>
  ...
  <Text as="p" sx={{color:'fg.muted',mb:4,fontSize:1}}>Manage how Reactor CMS looks to you. Select a theme and choose between light mode, dark mode, or follow your operating system preference.</Text>
  <Box className="appearance-section">
   ...
   <Box sx={{mt:3}}><SegmentedControl aria-label="Color mode" onChange={index=>setColorMode(modes[index])}><SegmentedControl.Button selected={colorMode==='light'} leadingIcon={SunIcon}>Light</SegmentedControl.Button><SegmentedControl.Button selected={colorMode==='dark'} leadingIcon={MoonIcon}>Dark</SegmentedControl.Button><SegmentedControl.Button selected={colorMode==='auto'} leadingIcon={DeviceDesktopIcon}>Auto</SegmentedControl.Button></SegmentedControl></Box>
  </Box>
  <h3 className="appearance-heading">Application theme</h3>
  <Text as="p" sx={{color:'fg.muted',mb:3,fontSize:1}}>Choose a Primer colour palette. Each preview adapts to the color mode selected above.</Text>
  <Box className="theme-grid">{themeVariants.map((variant:ThemeVariant)=>{const config=themeConfigs[variant],active=variant===theme;return <Box key={variant} as="button" className={`theme-card${active?' selected':''}`} onClick={()=>setTheme(variant,false)}>
  ...
  <h3 className="appearance-heading website-heading">Website theme</h3>
  <Text as="p" sx={{color:'fg.muted',mb:3,fontSize:1}}>Select the visual theme used by the public Astro website. This setting applies to every visitor.</Text>
