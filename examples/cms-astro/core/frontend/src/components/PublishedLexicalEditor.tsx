...
export type PublishedLexicalEditorProps = { body: string; lexical?: string };

export default function PublishedLexicalEditor({ body, lexical }: PublishedLexicalEditorProps) {
  ...
  return (
    <LexicalComposer initialConfig={initialConfig}>
      <div className="published-lexical">
        <RichTextPlugin
          contentEditable={<ContentEditable className="published-content-input" aria-label="Published article content" />}
          placeholder={null}
          ErrorBoundary={LexicalErrorBoundary}
        />
        <ListPlugin /><CheckListPlugin /><LinkPlugin /><HashtagPlugin />
        <CodeBlockHighlightPlugin /><TablePlugin /><CollapsiblePlugin />
        <EquationsPlugin /><ExcalidrawPlugin /><ImagesPlugin />
        <HorizontalRulePlugin /><YouTubePlugin />
      </div>
    </LexicalComposer>
  );
}
