...
[`PluginManifest`](/python-plugins/plugins), so a host listing both tiers never has to
...
The rule that pays for everything on the [lazy loading](/typescript-plugins/lazy-loading)
