...
transitively, exactly as [`deactivate`](/typescript-plugins/deactivation) does.
...
argument as [deactivation](/typescript-plugins/deactivation), one level up.
...
somebody to try. The [plugins manager](/core-plugins/manager) greys it and says why.
...
You rarely have to write that: [`@datalayer/reactor-manager`](/core-plugins/manager)
