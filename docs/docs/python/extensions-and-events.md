...
The [three states](/typescript/deactivation) are the same on both tiers.
...
Today an event fired on one tier activates plugins on that tier only. Making a
frontend plugin's activation carry to its backend counterpart, and back, is
[on the roadmap](/roadmap/cross-tier-activation).
